#!/usr/bin/env python3
"""
Long-context decision tests for a vLLM endpoint using the logprob method
(one forward pass, read the probability of each option label).

Setup:
    export RUNPOD_API_KEY="your-key"
    export ENDPOINT_URL="https://h2duge74u2oxgm.api.runpod.ai"   # optional, this is the default
    export MODEL="gemma4-e4b"                                    # optional, this is the default

Run:
    python longtest.py                 # run every test
    python longtest.py --only rules    # run tests whose name contains "rules"
    python longtest.py --dry-run       # build prompts and show sizes, no API calls
...
BASE = os.environ.get("ENDPOINT_URL", "https://h2duge74u2oxgm.api.runpod.ai").rstrip("/")
...
# --------------------------------------------------------------------------------------
# Building blocks for long, realistic states
# --------------------------------------------------------------------------------------
...
POLICY_FILLER = [
    ("Service availability", "We target 99.9% monthly uptime for the web application, excluding "
    ...
    ("Security", "All data is encrypted in transit and at rest. Access to customer data by staff requires "
     "a support ticket and is logged. Security incidents are reported to affected customers within 72 hours."),
    ("Seat management", "Seats can be added at any time and are prorated for the remainder of the billing "
     "period. Removing seats takes effect at the next renewal; removed seats are not refunded."),
    ("Taxes", "Prices exclude VAT and other applicable taxes, which are added to invoices based on the "
     "billing address. Customers are responsible for providing a valid VAT number where applicable."),
    ("Account closure", "Customers can close their account at any time from the settings page. Data is "
...
def q_noul(qid, text, expected):
    return {"id": qid, "type": "noul", "text": text,
            "options": [("yes", None), ("no", None)], "expected": expected}
...
def build_cases():
    ...
    billing_msg = ("Separate issue that still needs attention: I just checked our company card statement and "
    ...
    for length in (500, 2000, 4000, 6500):
        ...
        q = q_choice("routing", "Which team should handle the open issue in this thread that still needs action?",
                     ROUTING, "billing")
        cases.append({"case": "buried_fact", "variant": f"{length}", "state": state, "questions": [q]})
        if length == 2000:
            cases.append({"case": "buried_fact", "variant": "2000-reversed", "state": state,
                          "questions": [q], "reverse": True})

    # 2. Updated facts + several questions on one state (prefix caching)
    upd_state = build_thread(2500, [
    ...
    cases.append({"case": "updated_fact_multi", "variant": "2500", "state": upd_state, "multi": True,
                  "questions": [
                      q_choice("routing", "Which team should handle the issue that is currently open?",
                               ROUTING, "technical"),
                      q_noul("billing_unresolved", "Is there still an unresolved billing problem in this thread?",
                             "no"),
    ...
    for name, expected, record in records:
        state = policy + "\n\nCASE RECORD\n" + record
        q = q_noul("eligible", "Under the policy above, is the customer eligible for a refund of the charge "
                               "they are requesting?", expected)
    ...
    dis_state = build_thread(2000, [
        ...
        (0.5, "Customer (Alex Chen)", "Got the quote, thanks. Finance is reviewing it and we'll decide next "
    ...
    cases.append({"case": "distractor", "variant": "2000-reversed", "state": dis_state, "questions": [q],
                  "reverse": True})
    return cases


# --------------------------------------------------------------------------------------
# API call and probability processing
# --------------------------------------------------------------------------------------
...
def labels_for(q, reverse=False):
    ...
    return labels, [o[0] for o in opts], [o[1] for o in opts]
...
def call(messages):
    ...
    for attempt in range(6):
        ...
        try:
            ...
            if e.code in (429, 500, 502, 503, 504) or "no workers" in last.lower():
                print(f"    retry {attempt + 1}: {last}", file=sys.stderr)
...
def label_probs(resp, labels):
    """Sum probability over label variants (e.g. 'Yes', ' Yes', 'yes'), normalise over labels.
    Returns (normalised probs per label, coverage = share of total probability on the labels)."""
    top = resp["choices"][0]["logprobs"]["content"][0]["top_logprobs"]
    mass = {lab: 0.0 for lab in labels}
    for t in top:
        tok = t["token"].strip().lower().rstrip(".:)")
        for lab in labels:
            if tok == lab.lower():
    ...
    return {lab: m / coverage for lab, m in mass.items()}, coverage


def run_question(state, q, reverse=False):
    labels, ids, descs = labels_for(q, reverse)
    resp, latency = call(build_messages(state, q, labels, descs))
    probs_l, coverage = label_probs(resp, labels)
    probs = {i: probs_l[lab] for lab, i in zip(labels, ids)}
    pred = max(probs, key=probs.get)
    row = {"q": q["id"], "type": q["type"], "tokens": resp.get("usage", {}).get("prompt_tokens"),
           "ms": round(latency * 1000), "pred": pred, "p_pred": probs[pred], "coverage": coverage,
           "top_token": resp["choices"][0]["logprobs"]["content"][0]["token"], "probs": probs}
    if q["type"] == "score":
        ev = sum(p * float(i) for i, p in probs.items())
        row.update(expected=f">={q['min_expected']}", ev=round(ev, 2), p_exp=None,
                   correct=ev >= q["min_expected"])
    else:
        row.update(expected=q["expected"], p_exp=probs[q["expected"]], correct=pred == q["expected"])
    return row
...
def main():
    ...
    ap.add_argument("--dry-run", action="store_true", help="build prompts, print sizes, make no API calls")
    ap.add_argument("--out", default=f"results_{datetime.now():%Y%m%d_%H%M%S}.json")
    args = ap.parse_args()

    cases = [c for c in build_cases() if not args.only or args.only in c["case"]]

    if args.dry_run:
        for c in cases:
            q = c["questions"][0]
            labels, _, descs = labels_for(q, c.get("reverse", False))
            msg = build_messages(c["state"], q, labels, descs)[1]["content"]
            print(f"{c['case']:<20} {c['variant']:<20} ~{len(msg) // 4:>5} tokens  "
                  f"({len(c['questions'])} question(s))")
        print("\nExample prompt (last case, last 600 characters):\n" + msg[-600:])
        return
    ...
    run_question("Customer says: the app works fine.", q_noul("warm", "Is there a problem?", "no"))

    rows, multi_stats = [], []
    header = (f"{'case':<20}{'variant':<20}{'question':<20}{'tokens':>7}{'ms':>7}  "
              f"{'pred':<11}{'p(pred)':>8}{'p(exp)':>8}{'cover':>7}  ok")
    print("\n" + header + "\n" + "-" * len(header))

    for c in cases:
        try:
            if c.get("multi"):
                seq = [run_question(c["state"], q) for q in c["questions"]]
                t0 = time.perf_counter()
                with ThreadPoolExecutor(len(c["questions"])) as ex:
                    list(ex.map(lambda q: run_question(c["state"], q), c["questions"]))
                wall = round((time.perf_counter() - t0) * 1000)
                multi_stats.append({"case": c["case"], "sequential_ms": [r["ms"] for r in seq],
                                    "parallel_wall_ms": wall})
                results = seq
            else:
                results = [run_question(c["state"], q, c.get("reverse", False)) for q in c["questions"]]
        except RuntimeError as e:
            print(f"{c['case']:<20}{c['variant']:<20}ERROR: {e}")
            rows.append({"case": c["case"], "variant": c["variant"], "error": str(e)})
            continue
        for r in results:
            r.update(case=c["case"], variant=c["variant"])
            rows.append(r)
            pred = r["pred"] if r["type"] != "score" else f"ev={r['ev']}"
            print(f"{c['case']:<20}{c['variant']:<20}{r['q']:<20}{r['tokens'] or 0:>7}{r['ms']:>7}  "
                  f"{pred:<11}{fmt(r['p_pred']):>8}{fmt(r['p_exp']):>8}{fmt(r['coverage'], 2):>7}  "
    ...
    print("\nSUMMARY")
    if ok_rows:
        print(f"  Correct: {sum(r['correct'] for r in ok_rows)}/{len(ok_rows)}")
        low_cov = [r for r in ok_rows if r["coverage"] < 0.9]
        print(f"  Low label coverage (<0.90): {len(low_cov)}"
              + "".join(f"\n    {r['case']}/{r['variant']}/{r['q']}: {r['coverage']:.2f} "
                        f"(top token {r['top_token']!r})" for r in low_cov))
        confident_wrong = [r for r in ok_rows if not r["correct"] and r["p_pred"] > 0.9]
        print(f"  Confident mistakes (wrong with p>0.9): {len(confident_wrong)}"
              + "".join(f"\n    {r['case']}/{r['variant']}/{r['q']}: predicted {r['pred']} "
                        f"at {r['p_pred']:.3f}" for r in confident_wrong))
        print("  Latency by prompt size:")
        for r in sorted((r for r in ok_rows if r["case"] == "buried_fact"), key=lambda r: r["tokens"] or 0):
            print(f"    {r['tokens']:>6} tokens: {r['ms']} ms")
    for m in multi_stats:
        s = m["sequential_ms"]
        print(f"  Same state, {len(s)} questions: sequential {s} ms "
              f"(first {s[0]} ms, later avg {sum(s[1:]) // max(1, len(s) - 1)} ms); "
              f"parallel wall time {m['parallel_wall_ms']} ms")
    for name in ("buried_fact", "distractor"):
        pair = {r["variant"]: r for r in ok_rows if r["case"] == name}
        a, b = pair.get("2000"), pair.get("2000-reversed")
        if a and b:
            same = a["pred"] == b["pred"]
            print(f"  Option-order check ({name}): original={a['pred']}, reversed={b['pred']} -> "
                  f"{'consistent' if same else 'CHANGED WITH ORDER'}")

    with open(args.out, "w") as f:
        json.dump({"endpoint": BASE, "model": MODEL, "rows": rows, "multi": multi_stats}, f, indent=2)
